                        ...
                        <?php if(is_array($data)): foreach($data as $key=>$v): ?><div class="droptarget droptarget<?php echo ($v['id']); ?>">
                              <div>
                                <div class="btnBG">
                                    ...
                                    <div class="li6">
                                      <a href="javascript:;" type="<?php echo ($v['type']); ?>" state="<?php echo ($v['state']); ?>"  cateId="<?php echo ($v['id']); ?>" cateName="<?php echo ($v['name']); ?>" cateName_en="<?php echo ($v['name_en']); ?>" cateNum="<?php echo ($v['order_num']); ?>" onclick="edit(this)">修改</a>  | 
                                      <a href="javascript:if(confirm('确定删除？'))location='<?php echo U('delete',array('id'=>$v['id']));?>'">删除</a> |
                                      <a href="<?php echo U('Admin/Ziliao/index',array('nav'=>I('get.nav'),'cate'=>$v['id']));?>" > 文档维护</a>
                                  </div>
                                </div>
                              </div>
                            </div>
